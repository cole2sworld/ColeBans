...
public final class Check implements CBCommand {
	...
	public String run(final String[] args, final CommandSender sender) {
		if (!(new PermissionSet(sender).canLookup))
		...
		if (args.length < 1) {
			...
			if (bd.getType() == BanHandler.Type.PERMANENT) {
				sender.sendMessage(ChatColor.RED + "-- " + ChatColor.DARK_RED + victim
						+ ChatColor.RED + " --");
				...
				if (canIps) {
					sender.sendMessage(ChatColor.RED + "IP Address: " + ChatColor.DARK_RED
							+ (ip == null ? "Unknown" : ip.getHostAddress()));
				}
			}
			else if (bd.getType() == BanHandler.Type.TEMPORARY) {
				sender.sendMessage(ChatColor.YELLOW + "-- " + ChatColor.GOLD + victim
						+ ChatColor.YELLOW + " --");
				...
				timeRemaining /= 1000;
				timeRemaining /= 60;
				sender.sendMessage(ChatColor.YELLOW + "Time Remaining (Minutes): " + ChatColor.GOLD
						+ timeRemaining);
				if (canIps) {
					sender.sendMessage(ChatColor.YELLOW + "IP Address: " + ChatColor.GOLD
							+ (ip == null ? "Unknown" : ip.getHostAddress()));
				}
			}
			else if (bd.getType() == BanHandler.Type.NOT_BANNED) {
				sender.sendMessage(ChatColor.AQUA + "-- " + ChatColor.DARK_AQUA + victim
						+ ChatColor.AQUA + " --");
				sender.sendMessage(ChatColor.AQUA + "Ban Type: " + ChatColor.DARK_AQUA
						+ "Not Banned");
				if (canIps) {
