...
final class EventListener implements Listener {
	...
	public final void onPlayerPreLogin(final PlayerPreLoginEvent event) {
		final BanData bd = Main.instance.banHandler.getBanData(event.getName(), BanHandler.SYSTEM_ADMIN_NAME);
		...
		if (GlobalConf.get("allowTempBans").asBoolean()) {
			final Long tempBanTime = Main.instance.banHandler.getBanData(player, BanHandler.SYSTEM_ADMIN_NAME).getTime();
			if (tempBanTime > -1) {
	...
	public final void onPluginEnable(@SuppressWarnings("unused") final PluginEnableEvent event) {
		if (Main.instance.permissionsHandler == null) {
			final Plugin permissions = Main.instance.getServer().getPluginManager().getPlugin("Permissions");
			if (permissions != null) {
